        ...
        <el-col :xs="24" :sm="24" :md="16" :lg="16">
            <div class="content " style="min-height: 90vh; margin-left: 20px; margin-right: 20px;display: flex; flex-direction: column">
                <el-card style="overflow: auto; width: 100%; height: 100%; margin-top: 3rem;  background: #6F42C1; border: none; color: white">
                    <h1 style="padding-bottom: 1rem;">{{getLang(location, 'grStats')}} {{group.name}}</h1>
...
<script>
    ...
    export default {
        ...
        head(){
            return {
                title: this.getLang(this.location, 'grStats') + ' ' + this.group.name
            }
        },
        data(){
          return {
        ...
        async asyncData({store}) {
            try {
                const data = await store.dispatch('stats/graphGroup');
                return {
                    group: data.group,
                    dates: data.dates
                }
        ...
        methods: {
            async graphGroupByDate(){
                try {
                    const data = await this.$store.dispatch('stats/graphGroupByDate', {
                        ch_from: this.from,
                        ch_to: this.to,
                    });
